...
local ui = UI.new("454 Private V2.2", 5013109572)
...
ESPSection:addToggle("ESP Toggle", true, function(toggle)
    esp:Toggle(toggle)
end)
...
ESPSection:addToggle("Show Enemy Distance", false, function(toggle)
    esp.Names = toggle 
end)
...
ESPSection:addToggle("ESP Face Player", true, function(toggle)
...
HitboxSection:addSlider("Hitbox Transparency (Greater = Transparent)", 100, 0, 100, function(value)
    trans = (value / 100)
    --reapply hitboxes
    for _, v in pairs(workspace:GetDescendants()) do
        if v.Name == "soldier_model" and v:IsA("Model") and not v:FindFirstChild("friendly_marker") then
...
local tab = ui:addPage("Misc Beta", 5012544693)
local ESPSection = tab:addSection("Gun Settings")

ESPSection:addToggle("No Sway", true, function(toggle)
    function()
        hook_return(TASK_ENUM.INIT_FPV_SOL_FIREARM_SPREAD);
    end)

ESPSection:addToggle("Remove Camera Shake", false, function(toggle))function(
	hook_return(TASK_ENUM.INIT_FPV_SOL_FIREARM_SPREAD);
end)

ESPSection:addToggle("No Spread", false, function(toggle))function(
	hook_return(TASK_ENUM.INIT_FPV_SOL_FIREARM_SPREAD);
end)

ESPSection:addToggle("No bloom", false, function(toggle))function(
	hook_return(TASK_ENUM.INIT_FPV_SOL_FIREARM_SPREAD);
end)

HitboxSection:addSlider("Hitbox Transparency (Greater = Transparent)", 0, 0, 100, function(value))
...
othersettings:addButton("Fetch Discord link", function()
    setclipboard('https://discord.gg/7zv2GpBqPM')
end)
...
ui:Notify("Script made by TeamTwo!", "Join the Discord in the Settings page !!!")